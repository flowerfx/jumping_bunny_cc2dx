...
void MediaStreamer::Initialize(__in const WCHAR* url)
{
	...
	byte * dataPtr = NULL;
	//chien
	extern byte * GetDataFileGlobal(std::string name);
	extern size_t GetSizeFileGlobal(std::string name);
	auto dataS = GetDataFileGlobal(CCUnicodeToUtf8(url).c_str());
	if (dataS)
	{
		length = GetSizeFileGlobal(CCUnicodeToUtf8(url).c_str());
		//
		dataPtr = dataS;
	}
	else
