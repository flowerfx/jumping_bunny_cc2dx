...
namespace RKUtils
{
	...
	bool Ark::Initialise(const char* pAbsFilename)
	{
		...
		// Open ArkArchive
#ifdef OS_W8
		FILE* pArkFile = _wfopen(Win8Toolkit::ConvertUnicodedFolderPath(pAbsFilename).GetString(), L"rb");
#else
	...
	**/
	bool Ark::GetFileHandle(ArkFileMetadata& Meta, ArkFileHandle& outHandle)
	{
		...
		// Open ArkArchive
#ifdef OS_W8
		FILE* pStream = _wfopen(Win8Toolkit::ConvertUnicodedFolderPath(m_sArkFilename.GetString()).GetString(), L"rb");
#else
...
#ifdef OS_W8
					FILE* pFile = _wfopen(Win8Toolkit::ConvertUnicodedFolderPath(pAbsFilepath).GetString(), L"wb");
#else
	...
	bool Ark::LoadIndex()
	{
		...
		UnloadIndex();
#ifdef OS_W8
		FILE* pArkFile = _wfopen(Win8Toolkit::ConvertUnicodedFolderPath(m_sArkFilename.GetString()).GetString(), L"rb");
#else
